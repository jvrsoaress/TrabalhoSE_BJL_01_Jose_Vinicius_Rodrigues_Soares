...
void set_pattern(const char* pattern, uint8_t r, uint8_t g, uint8_t b) {
    ...
    if (strcmp(pattern, "V") == 0) {
        ...
        led_buffer[24] = 0; led_buffer[23] = 0; led_buffer[22] = 0; led_buffer[21] = 0; led_buffer[20] = 0; // linha 1
        led_buffer[15] = 1; led_buffer[16] = 0; led_buffer[17] = 0; led_buffer[18] = 0; led_buffer[19] = 1; // linha 2
        led_buffer[14] = 0; led_buffer[13] = 1; led_buffer[12] = 0; led_buffer[11] = 1; led_buffer[10] = 0; // linha 3
        led_buffer[5]  = 0; led_buffer[6]  = 0; led_buffer[7]  = 1; led_buffer[8]  = 0; led_buffer[9]  = 0; // linha 4
        led_buffer[4]  = 0; led_buffer[3]  = 0; led_buffer[2]  = 0; led_buffer[1]  = 0; led_buffer[0]  = 0; // linha 5
    } else if (strcmp(pattern, "X") == 0) {
        // padrão X (vermelho, obstáculo)
        led_buffer[24] = 1; led_buffer[23] = 0; led_buffer[22] = 0; led_buffer[21] = 0; led_buffer[20] = 1; // linha 1
        led_buffer[15] = 0; led_buffer[16] = 1; led_buffer[17] = 0; led_buffer[18] = 1; led_buffer[19] = 0; // linha 2
        led_buffer[14] = 0; led_buffer[13] = 0; led_buffer[12] = 1; led_buffer[11] = 0; led_buffer[10] = 0; // linha 3
        led_buffer[5]  = 0; led_buffer[6]  = 1; led_buffer[7]  = 0; led_buffer[8]  = 1; led_buffer[9]  = 0; // linha 4
        led_buffer[4]  = 1; led_buffer[3]  = 0; led_buffer[2]  = 0; led_buffer[1]  = 0; led_buffer[0]  = 1; // linha 5
    } else if (strcmp(pattern, "SETA") == 0) {
        // padrão seta (azul, modo pontos)
        led_buffer[24] = 1; led_buffer[23] = 0; led_buffer[22] = 0; led_buffer[21] = 0; led_buffer[20] = 1; // linha 1
        led_buffer[15] = 0; led_buffer[16] = 1; led_buffer[17] = 0; led_buffer[18] = 0; led_buffer[19] = 1; // linha 2
        led_buffer[14] = 0; led_buffer[13] = 0; led_buffer[12] = 1; led_buffer[11] = 0; led_buffer[10] = 1; // linha 3
        led_buffer[5]  = 0; led_buffer[6]  = 0; led_buffer[7]  = 0; led_buffer[8]  = 1; led_buffer[9]  = 1; // linha 4
        led_buffer[4]  = 1; led_buffer[3] = 1; led_buffer[2] = 1; led_buffer[1] = 1; led_buffer[0] = 1; // linha 5
    } else {
        // padrão apagado
        for (int i = 0; i < NUM_PIXELS; i++) {
            led_buffer[i] = 0;
...
void gpio_irq_handler(uint gpio, uint32_t events) {
    ...
    if (current_time - last_time > 200) { // debounce 200ms
        ...
        // joystick aciona Game Over em qualquer estado (exceto GAME OVER)
        if (gpio == SW_PIN && estado != GAME_OVER && !game_over_triggered) {
            estado = GAME_OVER;
...
int main() {
    ...
    while (true) {
        ...
        if (estado == MENU_INICIAL) {
            ...
            ssd1306_rect(&ssd, 0, 0, 128, 128, true, false); // borda em x=0, y=0, 
            ssd1306_draw_string(&ssd, "    STARDOG", 4, 16);
            ssd1306_draw_string(&ssd, "    JOGAR: A", 4, 36);
            ssd1306_draw_string(&ssd, "   PONTOS: B", 4, 46);
            ...
            printf("Desenhando no display: %s\n", max_pts_str); 
        ...
        if (current_time - last_print_time >= 1000) {
            printf("Nave: (%d, %d), PTS: %lu, Max: %lu, Estado: %d\n", nave_x, nave_y, pontuacao, pontuacao_max, estado); 
